...
public class Pesquisador
{
    ...
    {
        var Nomao = uni.Disciplinas;
        foreach (var item in Nomao)
        {
    ...
    {
        var Departs = uni.Departamentos;
        var Materias = uni.Disciplinas;
        foreach (var departo in Departs)
        {
            int materias = 0;
            foreach (var matero in Materias)
            {
                if(matero.DepartamentoID  == departo.ID)
                {
                    materias += 1;
                }
            }
            WriteLine($"{departo.Nome} tem {materias} disciplinas.");
            materias = 0;
        }
    }
    ...
    {
        var Luninho = uni.Alunos;
        foreach (var luno in Luninho)
        {
            WriteLine($"{luno.Nome}, idade: {luno.Idade}, professores: ");
        }
    }
